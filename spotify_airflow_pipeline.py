...
def _fetch_spotify_data(**kwargs):
    ...
    client_secret = Variable.get("spotify_secret_id")
    ...
    playlist_link = "https://open.spotify.com/playlist/3qQwZvSUjAxIi3BkKOhuMp"
...
def _read_data_from_s3(**kwargs):
    ...
    bucket_name = "spotify-etl-project-kavya-ariflow"
    ...
    for key in keys:
        if key.endswith(".json"):
            ...
            if data and data.strip():  # Check content is not empty
                try:
                    parsed = json.loads(data)
                    spotify_data.append(parsed)
                    print(f"Loaded file: {key}")
                except json.JSONDecodeError as e:
                    print(f"Failed to parse JSON from {key}: {e}")
            else:
                print(f"Skipped empty file: {key}")

    print(f"Total valid files loaded: {len(spotify_data)}")
...
store_raw_to_s3 = S3CreateObjectOperator(
    ...
    s3_bucket="spotify-etl-project-kavya-ariflow",
    s3_key="raw_data/to_processed/{{ ti.xcom_pull(task_ids='fetch_spotify_data', key='spotify_filename') }}",
